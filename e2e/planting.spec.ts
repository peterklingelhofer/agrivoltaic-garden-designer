...
test('applying a layout plants every bed, and another mix can be tried on any of them', async ({
  ...
  // the bed holds one of the combinations worked out for it, and says how sure that was
...
test('the plants step says what it planted and offers the ways on', async ({ page }) => {
  ...
  await expect(page.getByTestId('readout-step-summary-light')).toHaveText('Worked out')
  ...
  // field in view, which is the thing everyone in the newcomer audit wanted next
  const bedId =
