...
budget: the whole crop catalogue is about 31 kB gzipped and a few hundred kB is the ceiling for
bundled data.
...
grid does not cost a gigabyte of climate rasters to keep the file right. That is why the
generator, the date, the format version and whether it validated are recorded per layer: with two
scripts writing the file, none of those is true of it as a whole.
...
| NRCan zones rasterised at 0.03° | 4.1 MB | 374 kB | not measured |
| NRCan zones rasterised at 0.05° | 1.5 MB (1 769 × 830) | 181 kB | **75 kB** |
| NRCan zones rasterised at 0.06° | 1.0 MB | 136 kB | not measured |
...
9.3 km) aggregate the authors publish in the same archive. Shipping the authors' own aggregate
keeps the classification theirs end to end, with no resampling step here.
...
boundary. That is the cost of the budget.

Whether this beats the Open-Meteo derivation is a fair question, since ERA5-Land is about 9 km
too. It does, for two reasons that have nothing to do with resolution. Beck's inputs are
station-interpolated climatologies (WorldClim, CHELSA, CHPclim), where the derivation reads
reanalysis. And reanalysis precipitation bias falls directly on the aridity threshold and the
s/w/f seasonality letter, which is where a derived Köppen code is most likely to be wrong. It is
also the classification a user will find if they look their site up anywhere else.
...
**OPHZ is traced from the 2012 USDA map image.** Its own README says so. The
...
mean annual extreme minimum temperature grid, nearest-neighbour resampled to 0.02° and classified
into the published 5 °F half-zones.

Classifying after resampling: a mean of two temperatures across a zone boundary
...
those sites fall through to the derivation, which is the intended behaviour.

## 5. NRCan ships, and is still never crosswalked

The type was the blocker. `HardinessRating` cannot require `extremeMinTempC` of every scheme,
because the NRCan shapefile carries no temperature at all: only `ph_zone`, 0a to 9a. Filling that
field would mean assigning a USDA winter minimum to a zone that is not one.
...
1. a **temperature** rating from `derivedHardiness`: the mean of thirty annual extreme minima
   from ERA5 daily minima via Open-Meteo. It is the same physical quantity PRISM measures,
   computed from reanalysis with no crosswalk from another scheme, and it is what this code
   does everywhere the PRISM grid has no coverage, which includes all of Canada. It is the
   rating `climateGate` reads.
...
rings are then scan-converted with the even-odd rule at 0.01° and each 0.05° cell takes its centre
subcell, or the majority of its 25 subcells when the centre is uncovered and the cell is at least
half covered.
...
a source whose zones are broad generalised bands where PRISM is an 800 m surface. That is the
trade taken.
...
downstream says the native answer is unknown. That is the correct answer, and it is
...
probes. Without them a decoder bug is invisible: testing for the 8-bit no-data sentinel as well as
the 16-bit one regardless of the decoded width makes 255 no-data, and 255 is `PAL`, so every garden
in TDWG Palestine reads as "region unknown", permanently and indistinguishably from being at sea.
...
through the app's own `botanicalAreaAt`, so the decoder is in the loop with the rasteriser.
...
from 461 kB packed to 549 kB, up 19 %. The packed column is measured with `git hash-object -w` then
`git cat-file --batch-check '%(objectsize:disk)'`. All three grids are immutable and regenerable,
and a rebuild that changes a byte adds a second copy to history, so they should be rebuilt only
when an upstream actually revises.
...
gzipped. Every user pays the 75 kB Canadian layer, Canadian or not, exactly as every user
...
does not fill the grid exactly. Every one of those paths returns null and never throws, and
`koppenAt` and `hardinessAt` then derive from Open-Meteo exactly as before, with the same
...
An NRCan zone is never compared to either of them. It measures a different quantity, so there
is no disagreement to report, only two measurements of two different things,
listed separately.
...
OSU retains ownership, and set two conditions. Condition 2 is the one that binds this app: altered
data may be redistributed only if there is *"an explicit and prominently displayed disclaimer that
the map is not the official USDA Plant Hardiness Zone Map"* and the USDA-ARS and OSU logos are
eliminated. Resampling 800 m to 0.02° is an alteration. This app displays no logos, so the
...
grows, which is not a statement about the cultivar in a seed packet, and native is not a synonym
for easy to grow.
...
renders `staticLayerLicences()` and holds no hardcoded credit list, so every shipped layer
reaches the screen with its own licence text and the redistribution condition is met. One follow-up
remains: `docs/CITATIONS.csl.json` records `usda-phzm-2023` as `accessLevel: "public-domain"`, which
the terms of use above contradict. It needs reclassifying, and the OPHZ 2012 vintage belongs in the
`ophz-hardiness-geojson` caveat.
