 ...
 *
 * Every number below was checked against the cited primary source in this session. Two trials
...
describe('the Laub et al. 2022 anchor data, read against the primary paper', () => {
  ...
  // Read directly from the open-access PDF in this session, not from a secondary summary. The
  ...
  // in Table S2 itself, which lives in a separate supplementary .docx this session did not fetch,
  ...
  it('reproduces the ANOVA table (Table 1, final reduced model) to the decimal the paper prints', () => {
    ...
    // behind doc 02's claim that a linear "% shade = % yield loss" model is empirically wrong and
...
describe('Marrou et al. 2013 (lettuce, Montpellier FR, irrigated, not water-limited)', () => {
  ...
  // corroborated by two independent secondary sources in this session in addition to this
  // project's own doc 02 extraction: "the relative lettuce yield at harvest was equal or higher
...
describe('Barron-Gafford et al. 2019 (chiltepin, jalapeno, cherry tomato, Biosphere 2 AZ, water-limited)', () => {
  ...
  // manuscript in this session (osti.gov/servlets/purl/1567040). All three crops are Capsicum or
...
describe('Weselek et al. 2021 (potato and wheat, Heggelbach DE, ~30% RSR, drought vs normal year)', () => {
  ...
  it('potato, 2017 (normal year, not water-limited): the app predicts a bigger loss than the field trial measured', async () => {
    ...
    const curve = laubCurve(potato.laubGroup)
    const central = laubCentralRelativeYield(curve, RSR, false)
    // doc 02's own extraction gives potato a -20% to +11% range across both years; a normal-year
