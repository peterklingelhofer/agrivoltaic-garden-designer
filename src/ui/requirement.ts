 ...
 *
 * Four surfaces state one: `HandoffCard`, `rankingBlocker` with its `BLOCKED_BY` table,
 * `MissingRaster`, and the polyculture gate, which carries the sentence with no button. Each says
 * the same three things, so the shape names them: what is missing, why this panel cares, and the
 * press that fixes it.
 *
 ...
 * empty result whenever no light had been computed yet
 */
