 ...
 *
 * And not while the light is being worked out or is stale. A guided apply used to cost three
...
export const autoRunKey = (s: AppState): string =>
  [
    ...
    // the arrangement the light was worked out for, so a bake landing is one change and the
    ...
    /*
      ...
      plants step offers a press that raises the bed to fit. Left out, that press wrote the store,
      changed nothing this key could see, and the crop kept its EXCLUDED badge for good, with the
      remedy sentence going on to offer a raise of minus ten centimetres. A Master Gardener met
      exactly that and reported the badge as a liar, which it was
    */
    (s.plot?.beds ?? [])
      .map(
        (bed) =>
