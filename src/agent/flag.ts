 ...
 *
 * It arrives that way, because the value has to be statically
 * FOLDABLE, which correctness alone does not guarantee, and two earlier attempts were correct and did not fold: one
 ...
 * `src/vite-env.d.ts` for why it arrives already resolved, with `import.meta.env` never read
 * here directly
 */
