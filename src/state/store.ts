...
const plantBeds = (run: PlantingRun, set: Setter, get: () => AppState): void => {
  ...
  */
  ...
  /*
    ...
    combination score, small beside the agronomy on purpose, so pressing
    "sweet pepper" and then "basil" can make the pepper vanish from every bed: the top card in
    each bed no longer holds it. Each bed now takes the best fitting combination that covers the
    ...
    return {
      ...
      loadExample: async () => {
        ...
        set((s) => {
          ...
          // as stale and is computed again. Left null, the example's light outlived every
