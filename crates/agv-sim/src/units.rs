...
//! enough to check by eye against doc 02 section 1.3, which is why the tests below assert the
...
/// Diffuse skylight is blue-shifted, so it carries more photons per joule. Doc 03 section 2.6.
...
/// `DLI = GHI(MJ) x 1e6 J/MJ x f_PAR x 4.57 umol/J / 1e6 umol/mol`, doc 02 section 1.3.
...
mod tests {
    ...
    /// The worked example in doc 02 section 1.3: 20 MJ/m2/day at the default PAR fraction.
    #[test]
